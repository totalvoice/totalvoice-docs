...
curl 'https://api.totalvoice.com.br/conta' \
    ...
    -d '{"nome" : "Zenvia", "login" : "totalvoice@totalvoice.com.br", "senha" : "senha123"}'
```
```php
<?php
...
response = client.conta.criar_conta("Zenvia", "totalvoice@totalvoice.com.br", "senha123")
```
```java
...
```python
client = Cliente("seu-token", 'api.totalvoice.com.br')
response = client.conta.editar_conta("Zenvia", "totalvoice@totalvoice.com.br", "senha123")
```
```java
TotalVoiceClient client = new TotalVoiceClient("seu-token");
Conta conta = new Conta(client);
...
Este método permite a listagem de todos os webhooks default configurados para suas subcontas(Contas filhas). <a href="#webhooks">Clique aqui</a> para obter mais informações sobre cada webhook da ZenAPI. 
...
Este método permite a remoção de um webhook default específico.<a href="#webhooks">Clique aqui</a> para obter mais informações sobre cada webhook da ZenAPI. 
